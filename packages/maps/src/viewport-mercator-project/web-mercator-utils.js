...
export function getScaleByZoom(zoom) {
  const scale = zoomToScale(zoom);
  assert(Number.isFinite(scale));
  return scale * TILE_SIZE;
}
export function multiCoorLngLatToWorld([lng, lat], map) {
  assert(Number.isFinite(lng));
  assert(Number.isFinite(lat) && lat >= -90 && lat <= 90, 'invalid latitude');
  const [x, y] = transformToMultiCoor([lng, lat], map);
  return [x, y];
}
// 废弃了，提了新的方法在mapbox util里面
// function transformToMultiCoor(lng, lat, map, worldSize = 512) {
//   const coor = map.getCRS().fromWGS84([lng, lat]);
//   const extent = map.getCRS().getExtent();
//   const width = extent[2] - extent[0];
//   const height = extent[3] - extent[1];
//   console.log(width, height, extent)
//   // const lng1 = ((360 / width) * coor[0]) % 180;
//   // const lat2 = (170 / height) * coor[1];
//   const xScale = ((coor[0] - extent[0]) / width) * worldSize + 0;
//   const yScale = ((extent[3] - coor[1]) / height) * worldSize - worldSize/4;

//   // console.log(coor, extent, width, height, xScale, yScale);
...
export function worldToLngLat([x, y], scale, isGeographicCoordinateSystem) {
  ...
  var phi2;
  if (isGeographicCoordinateSystem === true) {
    phi2 = PI - (y * 2 * PI) / scale;
  } else {
    phi2 = 2 * (Math.atan(Math.exp(PI - (y / scale) * (2 * PI))) - PI_4);
  }
...
export function getProjectionParameters({
  ...
  const topHalfSurfaceDistance =
    ...
    Math.sin(Math.PI / 2 - pitchRadians - halfFov);

  // Calculate z value of the farthest fragment that should be rendered.
  const farZ =
    Math.cos(Math.PI / 2 - pitchRadians) * topHalfSurfaceDistance + altitude;
