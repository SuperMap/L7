 ...
 */
@injectable()
export default class MaplibreService extends BaseMapService<
  Map & IMaplibreInstance
> {
  public version: string = Version.MAPLIBRE;
  ...
  public async init(): Promise<void> {
    ...
    /**
     * TODO: 使用 maplibre v0.53.x 版本 custom layer，需要共享 gl context
     * @see https://github.com/maplibre/maplibre-gl-js/blob/master/debug/threejs.html#L61-L64
     */

    // 判断全局 maplibregl 对象的加载
    if (!mapInstance && !window.maplibregl) {
      // 用户有时传递进来的实例是继承于 maplibre 实例化的，不一定是 maplibregl 对象。
