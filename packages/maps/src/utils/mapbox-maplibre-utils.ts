...
export function fromWGS84(
  ...
  map: mapboxgl.Map | maplibregl.Map | maplibregl.Map,
) {
  if (!coor) return;
  const [lng, lat] = coor || [];
  // @ts-ignore
...
export function getCRSExtent(map: mapboxgl.Map | maplibregl.Map) {
  // @ts-ignore
...
// 当前级别一张瓦片代表的地理宽度
function getResolutionRatio(zoom: number, map: mapboxgl.Map | maplibregl.Map) {
  const extent = getCRSExtent(map);
  const width = extent[2] - extent[0];
  const height = extent[3] - extent[1];
